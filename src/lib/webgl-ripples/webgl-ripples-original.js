...
(function (global, factory) {
	...
	var Ripples = function (el, options) {
		...
		// Store the original CSS background before we potentially hide it
	...
	Ripples.prototype = {
		...
		computeTextureBoundaries: function () {
			...
			if (backgroundSize == 'cover') {
				...
				// Handle empty or missing background-size
				if (!backgroundSize || backgroundSize === '') {
					// Default to actual image size when no background-size is specified
					var backgroundWidth = this.backgroundWidth;
					var backgroundHeight = this.backgroundHeight;
				} else {
					...
					} else if (backgroundWidth != 'auto' && backgroundWidth !== '') {
						backgroundWidth = parseFloat(backgroundWidth);
					}

					if (isPercentage(backgroundHeight)) {
						backgroundHeight = (container.height * parseFloat(backgroundHeight)) / 100;
					} else if (backgroundHeight != 'auto' && backgroundHeight !== '') {
						backgroundHeight = parseFloat(backgroundHeight);
					}

					// Handle auto or empty values
					if (
						(backgroundWidth == 'auto' || backgroundWidth === '' || isNaN(backgroundWidth)) &&
						(backgroundHeight == 'auto' || backgroundHeight === '' || isNaN(backgroundHeight))
					) {
						backgroundWidth = this.backgroundWidth;
						backgroundHeight = this.backgroundHeight;
					} else {
						if (backgroundWidth == 'auto' || backgroundWidth === '' || isNaN(backgroundWidth)) {
							backgroundWidth = this.backgroundWidth * (backgroundHeight / this.backgroundHeight);
						}

						if (backgroundHeight == 'auto' || backgroundHeight === '' || isNaN(backgroundHeight)) {
		...
		initShaders: function () {
			...
			// Initialize default uniforms if not set yet
			if (!this.renderProgram.uniforms.topLeft) {
				this.renderProgram.uniforms.topLeft = new Float32Array([0, 0]);
				this.renderProgram.uniforms.bottomRight = new Float32Array([1, 1]);
				this.renderProgram.uniforms.containerRatio = new Float32Array([1, 1]);
